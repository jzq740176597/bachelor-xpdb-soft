...
namespace XPBD
{
	...
	public class SoftBodySimulationManager : MonoBehaviour
	{
		...
		void RebuildCollisionBuffers(float dt)
		{
			...
			if (_cpuShapes.Length     < count)            _cpuShapes     = new ShapeDescriptorCPU[count];
			if (_cpuVels.Length       < count)            _cpuVels       = new Vector3[count];
			if (_cpuFacePlanes.Length < _totalFacePlanes) _cpuFacePlanes = new Vector4[Mathf.Max(_totalFacePlanes, 1)];
		...
		{
			_shapesBuffer        ?.Release(); _shapesBuffer         = null;
			_shapeVelBuffer      ?.Release(); _shapeVelBuffer       = null;
			_meshFacePlanesBuffer?.Release(); _meshFacePlanesBuffer = null;
			_impulseBuffer       ?.Release(); _impulseBuffer        = null;
		}
		...
		{
			foreach (var b in _bodies)
				if (b && b.GetInstanceID() == instanceID) return b;
			return null;
		}
